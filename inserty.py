...
def view_wolna_linia(cursor):
    ...
    left join autobus on autobus.id_linia = przystanki_info.id_linia and autobus.numer_godziny = przystanki_info.numer_przejazdu  
...
def view_polaczone_dane(cursor):
    ...
    and numer_godziny = numer_przejazdu
...
def polacz_dane(id_kierowca, id_autobus, nr_linia, nr_godziny, cursor, connection):
    query = f"""
        SELECT * FROM polacz_dane({id_kierowca},{id_autobus},{nr_linia}, {nr_godziny})
    """
    cursor.execute(query)
    connection.commit()
